...
const ServiceTile: NextPage<ServiceProps> = ({ service }) => {
  ...
  return (
    <div
      className={`duration-50 project-tile relative flex w-full items-end justify-center flex-col gap-4 overflow-hidden border-b-2 border-black p-6 text-white transition-all ease-in ${
        visible
          ? "opacity-100 sm:opacity-90 sm:hover:opacity-100"
          : "opacity-40"
      }`}
    >
      <div className="item-center z-10 flex w-full flex-col justify-start max-h-min rounded-lg border-2 border-[#233329] bg-[#4D4730] p-6 text-white/90 shadow-2xl sm:items-end md:w-2/5 lg:w-1/3">
        <VisibilitySensor
