    ...
    >
      ...
      >
        <div className="item-center z-10 flex w-full flex-col justify-between rounded-lg border-2 border-[#233329] bg-[#4D4730] p-6 text-white/90 shadow-2xl sm:items-start md:w-4/5 lg:w-1/3 2xl:w-1/3 ">
          ...
          <div className="flex h-min flex-grow flex-wrap justify-center gap-10 rounded-lg p-3 w-full">
          ...
          <div className="flex h-min flex-wrap justify-center gap-5 p-2 w-full">
            <h3 className="z-10  w-full text-left text-3xl font-bold">Tech Stack</h3>
            {project?.technologies?.split(",").map((tech) => (
              <div key={tech} className="w-1/4 h-full text-center text-xl border-black border rounded-xl p-2 bg-white/20">
                {skillLogos[tech] && (
                  <Image
                    className="m-auto"
                    src={skillLogos[tech]}
